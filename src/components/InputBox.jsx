...
function InputBox({label, amount, onAmountChange, onCurrencyChange, currencyOptions = [], selectCurrency = "usd", amountDisable = false, currencyDisable = false, className = ""}) {
   const amountInputId = useId()
    ...
    return (
        <div className={`bg-white/40 p-3 text-black/85 rounded-lg text-sm flex ${className}`}>
            <div className="w-1/2">
            ...
            <div className="w-1/2 flex flex-wrap justify-end text-right">
                ...
                <select className="rounded-lg px-1 py-1 bg-gray-100/40 cursor-pointer outline-none" value={selectCurrency} onChange={(e) => onCurrencyChange && onCurrencyChange(e.target.value)}disabled={currencyDisable}>
